...
/**
 * Draw a black square filled with color.
 * 
 * @param canvas canvas to draw on
 * @param x x position of center of box
 * @param y y position of center of box
 * @param color index of COLORS which denotes the color that will be used to fill the box
 */
function drawBox(canvas: HTMLCanvasElement, x: number, y: number, color: number): void {
...
function main(): void {
    ...
    //     drawBox(canvas, event.offsetX, event.offsetY, 0);
    // });

    const puzzle = new Puzzle();
    for (let i = 0; i < puzzle.getRegions.length, i++) {
        const color = i;
        for (const cell of puzzle.getRegions[i]) {
            let x = cell.getX();
            let y = cell.getY();
